 ...
 */
if ( ! class_exists( 'Sov_Case_Management_Completed' ) ) {
	require_once ABSPATH . 'wp-admin/includes/class-wp-list-table.php';
}

class Sov_Case_Management_Completed extends WP_List_Table {
	...
	private function table_data() {
		...
		if ( isset( $_GET['s'] ) ) {
			...
			if ( ctype_digit( $search ) && (int) $search > 0 ) {
				$case_details = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}case_management_case WHERE case_status = 'Completed' AND case_id =  %d LIMIT 1", $search ) );
			} else {
				// value is string i.e Username/Shipping Address
				$case_details = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT * FROM {$wpdb->prefix}case_management_case WHERE case_status = 'Completed' AND (first_name LIKE %s OR last_name LIKE %s ) GROUP BY case_id",
				...
				$case_details = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}case_management_case WHERE case_status = 'Completed' GROUP BY case_id" );
		}
		...
		foreach ( $case_details as $case ) {
			...
			$action_link   = admin_url( 'admin.php?page=sov-case-management-dashboard&tab=completed&action=view&id=' . $caseID );
...
$wp_list_table = new Sov_Case_Management_Completed();
