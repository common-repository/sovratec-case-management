 ...
 */
if ( ! class_exists( 'Sov_Case_Management_Request' ) ) {
	require_once ABSPATH . 'wp-admin/includes/class-wp-list-table.php';
}

class Sov_Case_Management_Request extends WP_List_Table {
	...
	function column_orderID( $item ) {
		// Build row actions
		$actions = array(
			'view' => sprintf( '<a href="#" data-order_id="%s"  class="order-item-view" data-bs-toggle="modal" data-bs-target="#myModal_%s" >%s</a>', $item['caseID'], $item['caseID'], __( 'View Details', 'sov-case-management' ) ),
		);
	...
	private function table_data() {
		...
		if ( isset( $_GET['s'] ) ) {
			...
			if ( ctype_digit( $search ) && (int) $search > 0 ) {
				$case_details = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}case_management_case WHERE case_status = 'New' AND case_id = %d LIMIT 1", $search ) );
			} else {
				// value is string i.e Username/Shipping Address
				$case_details = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT * FROM {$wpdb->prefix}case_management_case WHERE case_status = 'New' AND (first_name LIKE %s OR last_name LIKE %s) GROUP BY case_id",
				...
				$case_details = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}case_management_case WHERE case_status = 'New' GROUP BY case_id" );
		}
		...
		foreach ( $case_details as $case ) {
			...
			$caseID        = $case->case_id;
			$caseType      = $this->CaseName( $case->case_type );
			...
			$action_link   = admin_url( 'admin.php?page=sov-case-management-dashboard&tab=new&action=view&id=' . $caseID );
			$caseRowAction = '<a href="' . $action_link . '" id="view-action" class="button button-primary button-large">View Details</a>';
			$data[]        = array(
				'caseID'      => $caseID,
				'caseType'    => $caseType,
				'Name'        => $Name,
...
$wp_list_table = new Sov_Case_Management_Request();
